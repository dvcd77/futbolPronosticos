...
export default function ShotsPredictor() {
  const { teamMatchCache, setTeamMatchCache, setApiStatus } = useApp();
  const teams = useApp().teams;
  ...
  async function runShots() {
    ...
    setError(''); setLoading(true); setResult(null); setPlayerResult(null); setScorers([]);
    ...
    try {
      ...
      // Try to fetch scorers for player shots
      if (hasApiKey()) {
        try {
          const scorersList = await fetchScorers('WC', 50);
          const relevantScorers = scorersList.filter(
            s => s.team?.id === homeId || s.team?.id === awayId
          );
          setScorers(relevantScorers.slice(0, 20));
        } catch { /* scorers not critical */ }
      }
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  function handlePlayerSelect(scorerId) {
    setSelectedScorer(scorerId);
    if (!scorerId || !result) { setPlayerResult(null); return; }
    const scorer = scorers.find(s => s.player?.id === Number(scorerId));
    if (!scorer) return;

    const teamId = scorer.team?.id;
    const teamLambda = teamId === homeId ? result.lambdaHome : result.lambdaAway;
    // Estimate team goal rate
    const teamMatches = teamMatchCache[teamId] ?? [];
    const validM = teamMatches.filter(m => m.score?.fullTime?.home != null && m.status === 'FINISHED');
    let teamGoalRate = teamId === homeId ? result.lambdaHome : result.lambdaAway;
    if (validM.length > 0) {
    ...
    const pr = playerShotsPrediction({
      ...
      position: scorer.player?.section ?? 'MF',
  ...
  const histogramData = result
    ? result.histograms?.homeShots?.slice(0, 25).map(d => ({ ...d, awayCount: 0 })) ?? []
    : [];
  const awayHistData = result?.histograms?.awayShots?.slice(0, 25) ?? [];
    ...
    <div className="max-w-6xl mx-auto px-4 py-6">
      ...
      {!result ? (
        <EmptyState icon="⚽" title="Selecciona un partido y pulsa Calcular"
          sub="Estimación basada en fuerza atacante/defensiva de cada equipo. Datos de disparos aproximados mediante modelos estadísticos." />
      ) : (
        <div style={{ display: 'grid', gap: 16 }}>
            ...
            {[
              ...
              return (
                <div key={side} className="card">
                  <div style={{ color, fontWeight: 700, marginBottom: 16, fontSize: 16 }}>
                  ...
                  <div style={{ fontSize: 11, color: '#3a5070', marginTop: 8 }}>
                    xG estimado: <span style={{ color, fontFamily: 'monospace' }}>{result[`lambda${side.charAt(0).toUpperCase()+side.slice(1)}`]?.toFixed(2)}</span>
                  </div>
                </div>
              );
            })}
          ...
          <div className="card">
            <SectionTitle sub="Distribución de disparos totales por simulación">Histograma de disparos</SectionTitle>
            <div style={{ height: 160 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={result.histograms?.homeShots?.filter(d => d.value <= 25) ?? []} barSize={12}>
                  ...
                  <YAxis hide />
                  <Tooltip formatter={(v) => [`${(v * 100).toFixed(1)}%`, 'Prob']}
                  ...
                  <Bar dataKey="prob" radius={[2, 2, 0, 0]}>
                    {(result.histograms?.homeShots ?? []).map((_, i) => (
                      <Cell key={i} fill="#00D4AA" opacity={0.7} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            ...
            <div style={{ fontSize: 11, color: '#3a5070', textAlign: 'center', marginTop: 4 }}>
              ⚠️ Estimación estadística · Los datos de disparos reales no están disponibles en el tier gratuito de football-data.org
            </div>
          </div>

          {/* Player shots (if scorers available) */}
          {scorers.length > 0 && (
            <div className="card">
              <SectionTitle sub="Estimación de disparos esperados para un jugador">Predicción de jugador</SectionTitle>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 12, alignItems: 'center', marginBottom: 16 }}>
                <div>
                  ...
                  <div style={{ position: 'relative' }}>
                    <select value={selectedScorer ?? ''} onChange={e => handlePlayerSelect(e.target.value || null)}>
                      <option value="">— Elegir jugador —</option>
                      {scorers.map(s => (
                        <option key={s.player?.id} value={s.player?.id}>
                          {s.player?.name} ({s.team?.shortName ?? s.team?.name}) — {s.goals} goles
                        </option>
                      ))}
                    </select>
          ...
          {!hasApiKey() && (
            <div style={{ fontSize: 12, color: '#3a5070', textAlign: 'center', padding: 12 }}>
              💡 Agrega tu API key en ⚙️ Configuración para obtener datos de goleadores reales
            </div>
          )}
        </div>
      )}
